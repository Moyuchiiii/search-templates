...
5. `.claude/skills/ui-ux-pro-max/` のデザインシステムデータ（色、タイポグラフィ、スタイル）を参考にし、視覚的に洗練されたレポートを目指す
...
> **デザイン原則**: テンプレートのCSSはベースラインであり、そのまま使うだけでなく、テーマやドメインに合わせてカスタマイズすることを推奨する。`.claude/skills/ui-ux-pro-max/` のデザインデータや `.claude/skills/theme-factory/` のテーマプリセットを活用してレポートのビジュアル品質を高めること。
...
テンプレートから生成されたレポートが「AIが書いた感」を出さないよう、以下を徹底する。

- **定型的な接続表現を避ける**: 段落の冒頭が「さらに、」「加えて、」「一方で、」「このように、」で始まるパターンが3回以上続いたら書き直す
- **括弧での補足説明を避ける**: 「○○（△△のこと）」のような括弧補足は使わず、文章の中に自然に組み込む
- **過剰な構造化を避ける**: すべての段落を箇条書きにしない。分析・考察は地の文で書く
- **一人称・感情表現を入れない**: 「注目したいのは」「興味深いことに」「驚くべきことに」は禁止
- **判断基準**: 「これをGoogleで検索したら、AIが書いた記事だとすぐバレるか？」と自問する。バレるなら書き直す
